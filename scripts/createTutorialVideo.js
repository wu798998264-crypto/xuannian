...
function sceneHtml(scene) {
  ...
  </style></head><body><div class="stage"><div class="brand"><div class="brand-mark">玄</div>玄念 3.0</div><div class="title">${scene.title}</div><div class="subtitle">${scene.subtitle}</div><ul class="bullets">${bullets}</ul><div class="mock">${mockFor(scene.mock)}</div>${marks}${cursor}<div class="caption">${scene.caption}</div></div></body></html>`;
}

const scenes = [
  { mock:'clipboard', title:'玄念 3.0 快速上手', subtitle:'把复制记录、常用收藏和桌面参考内容放到一个轻量工具里。', bullets:['剪切板：自动留档，防止信息丢失','收藏：高频内容一键复用','置顶便签：把参考内容贴在桌面上'], caption:'玄念 3.0 主要围绕剪切板、收藏、快捷窗口和置顶便签展开。', duration:8 },
  ...
  { mock:'summary', title:'最后总结', subtitle:'记住这四个入口，就能快速理解玄念。', bullets:['剪切板：全面留档','收藏：高频复用','快捷窗口：快速调用和自动粘贴','置顶便签：桌面参考'], caption:'玄念 3.0：记录、复用、置顶参考，减少重复操作。', duration:9 },
];
