...
async function run() {
  ...
  assert(indexSource.includes('id="mediaVideoSeek"') && indexSource.includes('formatMediaVideoTime') && mediaStyleSource.includes('.media-video-progress'), 'video previews need an always-visible seekable progress control with current and total time');
  ...
  assert(mainSource.includes("seek.setAttribute('aria-label', '视频播放进度')") && mainSource.includes("overlay.style.display = 'flex'"), 'embedded blob previews need the same seekable progress control');
  ...
  assert(indexSource.includes("task.status==='interrupted'") && indexSource.includes('api.cancelMediaDownloadTask(task.id)') && indexSource.includes('knownSize'), 'interrupted downloads must show file size and expose a removable task action');
