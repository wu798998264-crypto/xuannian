...
async function run() {
  ...
  assert(indexSource.includes('data-music-action="download"') && indexSource.includes('data-music-action="favorite"') && !indexSource.includes('data-music-format-choice'), 'each music result must expose direct download and favorite-download actions without a misleading quality chooser');
  ...
  assert(!indexSource.includes('高清音质') && indexSource.includes("downloadMediaMusicVersion(Number(action.dataset.musicIndex),action.dataset.musicAction==='favorite')"), 'music downloads must start directly without exposing an unverified high-quality option');
