...
async function run() {
  ...
  assert(indexSource.includes('id="mediaVideoQualitySelect"') && indexSource.includes('selectedActionId') && indexSource.includes("downloadParsedMediaVideo(favorite?'favorite':'download',collection,selectedAction?.id||selectedQualityIndex)"), 'Douyin media actions and legacy quality choices must reach the native download bridge');
  assert(mainSource.includes("{ id: 'video', label: '视频', kind: 'video' }") && mainSource.includes("{ id: 'backup', label: '备用下载', kind: 'video' }") && mainSource.includes("{ id: 'audio', label: '音频', kind: 'audio' }") && mainSource.includes("{ id: 'image', label: '封面图片', kind: 'image' }"), 'Douyin results must expose four plain download item labels');
  assert(mainSource.includes('await movePathAcrossVolumes(sourcePath, destination);') && mainSource.includes("parsed.capturedLocalPath = '';"), 'completed preview caches must move into the selected library instead of being downloaded or copied again');
  ...
  assert(mainSource.includes('CAPTCHA cannot and must not be automated') && mainSource.includes('verificationPending: true') && !mainSource.includes('music verification visibility retry'), 'first-use Cloudflare verification must be handed to the user immediately and resume the preserved portal session afterward');
