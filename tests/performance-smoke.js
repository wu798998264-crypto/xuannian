...
async function run() {
  ...
  assert(mainSource.includes('MEDIA_PORTAL_WORKER_WIDTH = 1280') && mainSource.includes('MEDIA_PORTAL_WORKER_HEIGHT = 900') && mainSource.includes('new BaseWindow({') && mainSource.includes('workerWindow.showInactive()'), 'hidden media workers need a full viewport fallback when the main window is minimized');
  assert(mainSource.includes('function attachMediaPortalViewToBackgroundMain') && mainSource.includes('mainWindow.contentView.addChildView(view, 0)') && mainSource.includes("mediaPortalViewHost = 'main-background'"), 'active media automation must keep its full viewport composited underneath the XuanNian interface');
  assert(mainSource.includes('MEDIA_PORTAL_RENDER_HEARTBEAT_MS = 800') && mainSource.includes('function startMediaPortalRenderHeartbeat') && mainSource.includes('view.webContents.capturePage({ x: 0, y: 0, width: 96, height: 64 })'), 'first-use media automation must continuously drive rendering until the request finishes');
  assert(mainSource.includes('clearMediaPortalRenderHeartbeat();') && mainSource.includes('startMediaPortalRenderHeartbeat(state);'), 'media rendering heartbeat must be scoped to the active automation lifecycle');
  assert(mainSource.includes('function activateMediaPortalAutomationView') && mainSource.includes('focusable: true') && mainSource.includes('workerWindow.focus()'), 'media automation must run in an activatable offscreen compositor because provider scripts ignore synthetic work in a non-focusable host');
  assert(mainSource.includes('backgroundActivationPhase') && mainSource.includes("activateMediaPortalAutomationView(view, state, 'trusted-submit')") && mainSource.includes('mainWindow.focus()'), 'each automation phase and trusted submit must activate the worker and then restore the XuanNian window focus');
  ...
  assert(indexSource.includes(`<title>玄念${packageVersion}</title>`), 'main window title must match the packaged version');
  assert(quickSource.includes(`<title>玄念${packageVersion}快捷面板</title>`), 'quick window title must match the packaged version');
  ...
  assert(mainSource.includes('revision = Math.max(0, Number(updateState.revision || 0)) + 1'), 'update state broadcasts must carry a monotonically increasing revision');
  assert(mainSource.includes('hasKnownAppUpdate()'), 'a known newer version must remain directly downloadable even if its status is stale');
  ...
  assert(indexSource.includes('function effectiveUpdateStatus(') && indexSource.includes('function applyUpdateState('), 'the update dialog must normalize newer versions and reject stale asynchronous state');
  ...
  assert(!indexSource.includes('id="mediaMusicFormats"') && indexSource.includes('id="mediaMusicResults"'), 'music results must not restore obsolete global format controls');
  assert(indexSource.includes('data-music-action="lyrics"') && indexSource.includes('data-music-action="download"') && indexSource.includes('data-music-action="favorite"') && !indexSource.includes('data-music-format-choice'), 'each music result must expose lyrics, direct download and favorite-download actions without a misleading quality chooser');
  assert(indexSource.includes('class="black-btn media-music-favorite-button"') && indexSource.includes('aria-label="下载并收藏"') && indexSource.includes("iconSvg('bookmark')"), 'favorite-download must use an accessible bookmark icon without changing its action');
  assert(indexSource.includes("api.showItemContextMenu('music-search-result'") && mainSource.includes("kind === 'music-search-result'"), 'right-clicking a music search result must expose playback, download, favorite-download and lyrics actions');
  assert(mainSource.includes("ipcMain.handle('media:downloadMusicLyrics'") && mainSource.includes('extractGequbaoLyrics') && mainSource.includes("mediaCollectionDirectory(downloadPath, 'audio')"), 'lyrics must be extracted from the selected result and saved beside downloaded music');
  assert(mainSource.includes('rememberCompletedMediaDownload(completedTask)') && mainSource.includes("taskId = `lyrics-${Date.now()}") && mainSource.includes("notifyMediaDownloadProgress(progressTask('downloading'))"), 'lyrics downloads must use the visible tracked-download pipeline and persist completed history');
  assert(mediaLibrarySource.includes("'ape', 'lrc'") && mediaLibrarySource.includes('lyrics: isLyricsPath(entry.name)') && indexSource.includes("lyrics?'歌词':'音乐'"), 'downloaded LRC files must appear in the music list as lyrics');
  assert(indexSource.includes('canFavorite:!(item.lyrics') && mainSource.includes("options.canFavorite !== false") && mediaLibrarySource.includes("if (isLyricsPath(source)) return { ok: false"), 'lyrics must not expose or accept favorite actions');
  assert(indexSource.includes("mediaKindIcon(lyrics?'lyrics':item.kind)") && indexSource.includes("kind==='lyrics'") && mediaStyleSource.includes('.media-kind.lyrics'), 'lyrics rows must use a distinct document icon and color treatment');
  assert(mainSource.includes('async function openLocalFile(filePath)') && mainSource.includes("{ command: 'notepad.exe'") && mainSource.includes("args: ['-a', 'TextEdit', target]") && mainSource.includes('if (isLyricsPath(target) && await openLyricsInTextEditor(target))'), 'lyrics must open in a text editor without relying on an LRC file association');
  ...
  assert(!indexSource.includes('高清音质') && indexSource.includes('handleMediaMusicResultAction(Number(action.dataset.musicIndex),action.dataset.musicAction)'), 'music result actions must route directly without exposing an unverified high-quality option');
  ...
  assert(mediaStyleSource.includes('.media-music-results{height:100%;min-height:0;overflow:auto;padding:2px;'), 'music results must not reserve an obsolete blank header row');
  ...
  assert(stickySource.includes('.project-slot{flex:1;min-width:0;height:100%') && stickySource.includes('-webkit-app-region:drag') && stickySource.includes('button,input,textarea,select'), 'the sticky title bar must remain draggable around the interactive category selector and action buttons');
  assert(indexSource.includes('.modal-backdrop.note-editor-backdrop{inset:1px 1px 1px 63px') && indexSource.includes('class="center-title-modal note-editor-pane"') && indexSource.includes("`,'note-editor')"), 'the favorite editor must fill the workspace to the right of the main navigation rail');
  ...
  assert(mainSource.includes('music verification visibility retry') && mainSource.includes('visibleMsOverride: 2500') && mainSource.includes('rerunAfterWake: true'), 'first-use Cloudflare verification must be rechecked promptly while the active offscreen page remains available');
  assert(mainSource.includes('function pollMediaPortalVerificationClear') && mainSource.includes('music verification cleared early'), 'first music search must resume as soon as verification clears instead of always waiting the full visibility interval');
