...
async function run() {
  ...
  assert.deepStrictEqual(bilibili.portals.map((route) => route.label), ['Seekin', 'Seekin Bilibili']);
  ...
  assert.deepStrictEqual(detectVideoProvider('https://xhslink.com/example').portals.map((route) => route.label), ['Seekin', 'Xiaohongshua']);
  ...
  assert.deepStrictEqual(kuaishou.portals.map((route) => route.label), ['Seekin', 'HelloTik']);
  ...
  try {
    ...
    fs.writeFileSync(path.join(collectionPath, '说明.txt'), 'keep this file');
    fs.writeFileSync(path.join(nestedPath, 'live.mp3'), Buffer.alloc(24));
    const removed = await deleteMediaCollection(downloads, 'audio', '常用配乐');
    assert.strictEqual(removed.ok, true);
    assert.strictEqual(removed.moved, 1);
    assert.strictEqual(removed.preserved, 3);
    assert.strictEqual(removed.folders, 1);
    assert.strictEqual(removed.otherFiles, 1);
    assert.strictEqual(fs.existsSync(path.join(downloads, '音乐', 'song.mp3')), true);
    assert.strictEqual(fs.existsSync(path.join(downloads, '音乐', '说明.txt')), true, 'non-media files must be preserved when deleting a collection');
    assert.strictEqual(fs.existsSync(path.join(downloads, '音乐', '现场录音', 'live.mp3')), true, 'nested folders must be preserved when deleting a collection');
    collections = await listMediaCollections(downloads);
