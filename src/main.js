...
const { writeVideoCover } = require('./media-cover');
...
const mediaPortalPreviewPromises = new WeakMap();
...
// Preview uses the same full media source as the eventual download. A zero limit
// avoids silently selecting a lower-quality substitute or rejecting a valid source.
const MEDIA_PREVIEW_MAX_BYTES = 0;
const MEDIA_PREVIEW_START_TIMEOUT_MS = 25 * 1000;
const MEDIA_PREVIEW_DOWNLOAD_TIMEOUT_MS = 10 * 60 * 1000;
...
function looksLikeTextualMediaPayload(value) {
  const buffer = Buffer.isBuffer(value) ? value : Buffer.from(value || '');
  if (!buffer.length) return false;
  const sample = buffer.subarray(0, 512).toString('utf8').replace(/^\uFEFF/, '').trimStart();
  return /^(?:<!doctype\s+html|<html(?:\s|>)|<head(?:\s|>)|<body(?:\s|>)|<\?xml|\{\s*["']|\[\s*\{)/i.test(sample);
}

function isUsableMediaResponseChunk(value, contentType = '') {
  const type = String(contentType || '').toLowerCase();
  if (/^(?:text\/|application\/(?:json|xml|xhtml\+xml))/i.test(type)) return false;
  return !looksLikeTextualMediaPayload(value);
}

function isDirectMediaActionUrl(value) {
  const url = sanitizeRemoteMediaUrl(value);
  if (!url) return false;
  if (isMediaUrl(url)) return true;
  try {
    return !!mediaKindForPath(new URL(url).pathname);
  } catch {
    return false;
  }
}
...
function mediaDownloadReferer(value, fallback = '') {
  try {
    ...
    if (/(?:^|\.)(?:douyinvod\.com|bytev\.com|douyinpic\.com|snssdk\.com|zjcdn\.com)$/i.test(parsed.hostname)) return 'https://www.douyin.com/';
...
function streamNodeMediaPortalUrlToFile(url, destination, options = {}, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    request = client.get(value, {
      ...
      const hintedKind = ['video', 'audio', 'image'].includes(String(options.kindHint || '')) ? String(options.kindHint) : '';
      const textualResponse = /^(?:text\/|application\/(?:json|xml|xhtml\+xml))/i.test(contentType);
      const kind = textualResponse ? '' : (mediaKindForDownloadUrl(value)
        || (/^video\//i.test(contentType) ? 'video' : (/^audio\//i.test(contentType) ? 'audio' : (/^image\//i.test(contentType) ? 'image' : '')))
        || (hintedKind && isMediaUrl(value) ? hintedKind : ''));
      if (response.statusCode < 200 || response.statusCode >= 300) {
        response.resume();
      ...
      response.on('data', (chunk) => {
        if (firstChunk) {
          firstChunk = false;
          if (!isUsableMediaResponseChunk(chunk, contentType)) {
            const error = new Error(`unexpected media response ${contentType || 'textual payload'}`);
            try { response.destroy(error); } catch {}
            try { request.destroy(error); } catch {}
            finish(error);
            return;
...
async function streamMediaPortalUrlToFile(webContents, url, destination, options = {}) {
  ...
  try {
    ...
    const hintedKind = ['video', 'audio', 'image'].includes(String(options.kindHint || '')) ? String(options.kindHint) : '';
    const textualResponse = /^(?:text\/|application\/(?:json|xml|xhtml\+xml))/i.test(contentType);
    const kind = textualResponse ? '' : (mediaKindForDownloadUrl(resolvedUrl)
      || mediaKindForDownloadUrl(url)
      || (/^video\//i.test(contentType) ? 'video' : (/^audio\//i.test(contentType) ? 'audio' : (/^image\//i.test(contentType) ? 'image' : '')))
      || (hintedKind && isMediaUrl(resolvedUrl || url) ? hintedKind : ''));
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    ...
    while (true) {
      ...
      if (firstChunk) {
        firstChunk = false;
        if (!isUsableMediaResponseChunk(buffer, contentType)) {
          throw new Error(`unexpected media response ${contentType || 'textual payload'}`);
        }
  ...
  try {
    ...
    const result = await streamMediaPortalUrlToFile(webContents, url, destination, {
      ...
      onStarted: ({ totalBytes }) => {
        ...
        clearTimeout(captureState.startTimeout);
...
async function startDirectTrackedMediaDownload(webContents, url, referer) {
  ...
  const kindHint = ['video', 'audio', 'image'].includes(String(target.kindHint || '')) ? String(target.kindHint) : '';
  let extension = kindHint === 'image' ? '.jpg' : (kindHint === 'audio' ? '.mp3' : '.mp4');
  ...
  const kind = mediaKindForPath(filename) || kindHint || 'video';
...
function startCapturedMediaPortalDownload(webContents, url) {
  const safeUrl = sanitizeRemoteMediaUrl(url);
  if (!isDirectMediaActionUrl(safeUrl)) {
    runtimeLog(`ignored non-media portal download source=${String(url || '').slice(0, 260)}`);
    return null;
  }
  ...
  direct.then((result) => {
    ...
    if (previewCapture) {
      runtimeLog('direct media preview request did not start; preview remains unavailable');
    ...
    try { webContents.downloadURL(safeUrl); } catch {}
...
function configureMediaDownloadSession(electronSession) {
  ...
  electronSession.on('will-download', (event, item, webContents) => {
    if (mediaPortalPreviewCapture?.webContents === webContents) {
      ...
      const textualMime = /^(?:text\/|application\/(?:json|xml|xhtml\+xml))/i.test(mimeType);
      const kind = textualMime ? '' : (filenameKind || urlKind || (/^video\//i.test(mimeType) ? 'video' : ''));
      if (kind !== 'video') {
        event.preventDefault();
      ...
      if (MEDIA_PREVIEW_MAX_BYTES && initialBytes > MEDIA_PREVIEW_MAX_BYTES) {
        event.preventDefault();
      ...
      const updatePreviewProgress = () => {
        ...
        if (MEDIA_PREVIEW_MAX_BYTES && (receivedBytes > MEDIA_PREVIEW_MAX_BYTES || totalBytes > MEDIA_PREVIEW_MAX_BYTES)) {
    ...
    const urlKind = mediaKindForDownloadUrl(item.getURL());
    const textualMime = /^(?:text\/|application\/(?:json|xml|xhtml\+xml))/i.test(mimeType);
    const kind = textualMime ? '' : (filenameKind || urlKind
      || (/^video\//i.test(mimeType) ? 'video' : (/^audio\//i.test(mimeType) ? 'audio' : (/^image\//i.test(mimeType) ? 'image' : ''))));
    if (!kind) {
      item.cancel();
      notifyMediaDownloadsChanged({ status: 'blocked', message: '已拦截非媒体文件下载' });
      return;
    }
    markMediaPortalTransferStarted(webContents);
    ...
    if (String(target.preferredName || '').trim()) {
      const extension = filenameKind === kind
        ...
        : (urlKind === kind ? mediaExtensionForDownloadUrl(item.getURL(), kind === 'audio' ? '.mp3' : (kind === 'image' ? '.jpg' : '.mp4')) : (kind === 'audio' ? '.mp3' : (kind === 'image' ? '.jpg' : '.mp4')));
      ...
      const extension = kind === 'audio' ? '.mp3' : (kind === 'image' ? '.jpg' : '.mp4');
...
function scheduleMediaPortalIdleDestroy() {
  if (mediaPortalIdleTimer || mediaPortalPreviewCapture || !mediaPortalView || mediaPortalView.webContents.isDestroyed()) return;
  mediaPortalIdleTimer = setTimeout(() => {
    mediaPortalIdleTimer = null;
    if (mediaPortalPreviewCapture) {
      scheduleMediaPortalIdleDestroy();
      return;
    }
...
function startMediaPortalMusicResultSync(state, view = mediaPortalView) {
  ...
  const poll = async () => {
    ...
    if (Number(state.musicResultSyncPollCount || 0) === 5 || Number(state.musicResultSyncPollCount || 0) === 10) {
      // Some first-load pages only flush search rows after a compositor frame.
      // Wake the embedded portal briefly, then return to the native result view.
      activateMediaPortalInContentRegion(view, 260, 'music result sync');
    }
    if (Number(state.musicResultSyncPollCount || 0) === 6 || Number(state.musicResultSyncPollCount || 0) === 11) {
      try {
        const result = await view.webContents.executeJavaScript(buildPortalScript({
          mode: 'music-search',
          value: state.value || '',
        ...
        if (result?.ok && Array.isArray(result.results) && result.results.length) {
          runtimeLog(`music result sync fallback hit request=${state.requestId} poll=${state.musicResultSyncPollCount} count=${result.results.length}`);
          await completeMediaPortalAutomation(state, result);
          return;
        }
      } catch {}
...
function clearMediaPortalPreviewCapture(result = null) {
  ...
  clearTimeout(capture.startTimeout);
...
async function prepareMediaPortalVideoPreview(state, parsed) {
  ...
  clearMediaPortalPreviewCapture();
  const beginCapture = () => new Promise((resolve) => {
    clearMediaPortalPreviewCapture();
    cancelMediaPortalIdleDestroy();
    keepMediaPortalWorkerVisible(view);
    const capture = {
    ...
    capture.startTimeout = setTimeout(() => {
      if (mediaPortalPreviewCapture === capture && !capture.started) clearMediaPortalPreviewCapture(null);
    }, MEDIA_PREVIEW_START_TIMEOUT_MS);
    capture.timeout = setTimeout(() => {
      if (mediaPortalPreviewCapture === capture) clearMediaPortalPreviewCapture(null);
    }, MEDIA_PREVIEW_DOWNLOAD_TIMEOUT_MS);
    mediaPortalPreviewCapture = capture;
  });
  const captureDirectUrl = async (url, label = '临时预览') => {
    if (!isDirectMediaActionUrl(url)) return null;
    const capturedDownload = beginCapture();
    startCapturedMediaPortalDownload(view.webContents, url);
    const captured = await capturedDownload;
    if (captured) captured.label = String(label || '临时预览');
    return captured;
  };
  try {
    const nativePreviewUrl = sanitizeRemoteMediaUrl(parsed.previewUrl);
    if (nativePreviewUrl) {
      const captured = await captureDirectUrl(nativePreviewUrl, '原生预览');
      if (captured) return captured;
    }
    const actionVideo = Array.isArray(parsed.mediaActions)
      ? parsed.mediaActions.find((action) => action?.id === 'video' && action.href)
      : null;
    const actionBackup = Array.isArray(parsed.mediaActions)
      ? parsed.mediaActions.find((action) => action?.id === 'backup' && action.href)
      : null;
    const actionPreview = actionVideo || actionBackup;
    if (actionPreview?.href !== nativePreviewUrl && isDirectMediaActionUrl(actionPreview?.href)) {
      const captured = await captureDirectUrl(actionPreview.href, actionVideo ? '视频预览' : '备用预览');
      if (captured) return captured;
    }
    ...
    if (isMediaUrl(directPreviewOption?.href)) {
      const captured = await captureDirectUrl(directPreviewOption.href, directPreviewOption.label || parsed.qualityLabel || '临时预览');
      if (captured) return captured;
    }
    if (!parsed.downloadActionReady) {
      ...
      return captureDirectUrl(previewUrl, parsed.qualityLabel || '临时预览');
    }
    ...
    const candidateIndex = Number.isInteger(actionVideo?.candidateIndex) && actionVideo.candidateIndex >= 0
      ? actionVideo.candidateIndex
      : (Number.isInteger(directPreviewOption?.index)
    ...
    runtimeLog(`media preview candidate=${candidateIndex + 1}/${Math.max(1, Number(result?.candidateCount || parsed.candidateCount || 1))} ok=${!!result?.ok} native=${!!result?.nativeClickRequired} href=${isDirectMediaActionUrl(result?.href) ? 'media' : 'none'} label=${String(result?.label || '').slice(0, 160)}`);
    if (result?.nativeClickRequired) {
      await dispatchTrustedMediaPortalDownloadClick(view, result.actionPoint);
    }
    const href = sanitizeRemoteMediaUrl(result?.href);
    if (href && result?.ok && isDirectMediaActionUrl(href)) {
      startCapturedMediaPortalDownload(view.webContents, href);
    } else if (href && result?.ok) {
      runtimeLog(`ignored non-media preview candidate href=${href.slice(0, 260)}`);
    }
    if (!result?.ok) {
      clearMediaPortalPreviewCapture();
      return null;
...
function sanitizeParsedMediaActions(actions, sourceUrl = '') {
  if (detectVideoProvider(sourceUrl)?.id !== 'douyin') return [];
  const expected = [
    { id: 'video', label: '视频', kind: 'video' },
    { id: 'backup', label: '备用下载', kind: 'video' },
    { id: 'audio', label: '音频', kind: 'audio' },
    { id: 'image', label: '封面图片', kind: 'image' },
  ];
  const incoming = new Map((Array.isArray(actions) ? actions : []).map((action) => [String(action?.id || ''), action]));
  const seenUrls = new Set();
  return expected.map((definition) => {
    const candidate = incoming.get(definition.id) || {};
    let href = sanitizeRemoteMediaUrl(candidate.href);
    if (href && seenUrls.has(href)) href = '';
    if (href) seenUrls.add(href);
    return {
      ...definition,
      href,
      candidateIndex: Number.isInteger(candidate.candidateIndex) ? candidate.candidateIndex : -1,
    };
  });
}
...
function publicMediaPortalVideoPayload(parsed) {
  const {
    ...
    promotedLocalPath: _promotedLocalPath,
    ...publicParsed
  } = parsed;
  const reusableLocalPath = [parsed.capturedLocalPath, parsed.promotedLocalPath]
    .map((value) => String(value || ''))
    .find((value) => value && fs.existsSync(value));
  return { ...publicParsed, previewCached: !!reusableLocalPath };
}

function applyCapturedMediaPortalPreview(parsed, captured) {
...
async function completeMediaPortalAutomation(state, result = {}) {
  ...
  if (mode === 'video-parse') {
    ...
    const parsed = {
      ...
      title: sanitizeMediaVideoTitle(result.title, state.originalSourceText || state.value),
      ...
      mediaActions: sanitizeParsedMediaActions(result.mediaActions, state.value),
    ...
    runtimeLog(`media portal parsed result preview=${parsed.previewUrl ? (isDirectMediaActionUrl(parsed.previewUrl) ? 'media' : 'non-media') : 'none'} actions=${parsed.mediaActions.map((action) => `${action.id}:${action.href && isDirectMediaActionUrl(action.href) ? 'media' : (Number(action.candidateIndex ?? -1) >= 0 ? 'candidate' : 'none')}`).join(',')}`);
    ...
    if (parsed.ok && parsed.downloadReady) {
      ...
      const previewPromise = prepareMediaPortalVideoPreview(state, parsed).then((captured) => {
        applyCapturedMediaPortalPreview(parsed, captured);
        return captured;
      });
      mediaPortalPreviewPromises.set(parsed, previewPromise);
      try {
        await previewPromise;
      } finally {
        mediaPortalPreviewPromises.delete(parsed);
      }
      if (state.requestId !== mediaPortalRequestId) return;
      ...
      if (!mediaPortalInputState && !activeMediaPortalDownloads) scheduleMediaPortalIdleDestroy();
      return;
    }
    publishMediaPortalVideo(state, parsed);
  } else if (mode === 'music-search') {
...
async function reloadParsedVideoDownloadPage(view, parsed) {
  ...
  try {
    ...
    return {
      ...
      mediaActions: sanitizeParsedMediaActions(result.mediaActions, parsed.sourceUrl),
      downloadActionReady: !!result.downloadActionReady,
    };
  } catch (error) {
...
async function promoteMediaPreviewToLibrary(parsed, downloadTarget = 'download', collection = '') {
  const cachedSourcePath = String(parsed?.capturedLocalPath || '');
  const promotedSourcePath = String(parsed?.promotedLocalPath || '');
  const sourcePath = [cachedSourcePath, promotedSourcePath].find((value) => value && fs.existsSync(value)) || '';
  ...
  try {
    ...
    fs.mkdirSync(downloadPath, { recursive: true });
    if (promotedSourcePath && path.dirname(promotedSourcePath) === downloadPath) {
      return { ok: true, path: promotedSourcePath, previewFallback: true, completed: true, reusedLocal: true };
    }
    const destination = uniqueMediaDownloadPath(downloadPath, filename);
    ...
    if (sourcePath === cachedSourcePath) {
      await movePathAcrossVolumes(sourcePath, destination);
      parsed.capturedLocalPath = '';
    } else {
      await fs.promises.copyFile(sourcePath, destination, fs.constants.COPYFILE_EXCL);
    }
    parsed.promotedLocalPath = destination;
    parsed.previewUrl = pathToFileURL(destination).href;
    ...
    showMediaDownloadNotification({ status: 'completed', name: filename, filePath: destination, favorite: favoriteDownload });
    updatePublishedMediaPortalVideo(parsed);
...
async function saveMediaPreviewCoverToLibrary(parsed, downloadTarget = 'download', collection = '') {
  let sourcePath = [parsed?.capturedLocalPath, parsed?.promotedLocalPath]
    .map((value) => String(value || ''))
    .find((value) => value && fs.existsSync(value));
  try {
    if (!sourcePath) {
      const sourceAction = (Array.isArray(parsed?.mediaActions) ? parsed.mediaActions : [])
        .find((action) => action?.kind === 'video' && action.href && isDirectMediaActionUrl(action.href));
      const view = ensureMediaPortalView();
      if (!sourceAction || !view || view.webContents.isDestroyed()) {
        return { ok: false, code: 'cover-unavailable', reason: '当前结果无法生成封面图片' };
      }
      const extension = mediaExtensionForDownloadUrl(sourceAction.href, '.mp4');
      sourcePath = mediaPreviewCachePath(parsed.sourceUrl, extension);
      await streamMediaPortalUrlToFile(view.webContents, sourceAction.href, sourcePath, {
        referer: mediaDownloadReferer(sourceAction.href, parsed.portalUrl),
      });
      parsed.capturedLocalPath = sourcePath;
      parsed.capturedFilename = path.basename(sourcePath);
    }
    const directories = mediaDirectories();
    const favoriteDownload = downloadTarget === 'favorite';
    const rootPath = favoriteDownload ? directories.favoritePath : directories.downloadPath;
    const cleanTitle = sanitizeMediaVideoTitle(parsed.title, parsed.sourceUrl);
    const filename = sanitizeDownloadFilename(`${cleanTitle || '视频'}-封面.png`, 'image/png');
    const downloadPath = mediaCollectionDirectory(rootPath, 'video', String(collection || '').trim());
    fs.mkdirSync(downloadPath, { recursive: true });
    const destination = uniqueMediaDownloadPath(downloadPath, filename);
    const taskId = `media-cover-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
    const cover = await writeVideoCover(sourcePath, destination, 1280);
    const completedTask = {
      id: taskId,
      name: filename,
      path: destination,
    ...
    notifyMediaDownloadsChanged({
      ...
      path: destination,
      favorite: favoriteDownload,
      message: favoriteDownload ? '封面图片已保存到收藏目录' : '封面图片已保存到下载目录',
    });
    showMediaDownloadNotification({ status: 'completed', name: filename, filePath: destination, favorite: favoriteDownload });
    runtimeLog('generated cached media cover at ' + destination);
    return { ok: true, path: destination, generatedCover: true, completed: true };
  } catch (error) {
    runtimeLog('generate media preview cover failed: ' + (error?.message || error));
    return { ok: false, code: 'cover-unavailable', reason: '暂时无法从视频生成封面图片' };
  }
}

async function downloadParsedMediaVideo(downloadTarget = 'download', collection = '', selection = 0) {
  const parsed = mediaPortalParsedVideo;
  if (!parsed?.downloadReady) return { ok: false, code: 'parse-required', reason: '请先完成视频解析' };
  const actionId = typeof selection === 'string' ? String(selection || '').trim() : '';
  const mediaActions = Array.isArray(parsed.mediaActions) ? parsed.mediaActions : [];
  const selectedAction = mediaActions.find((action) => action?.id === actionId) || null;
  const selectedKind = String(selectedAction?.kind || 'video');
  ...
  const selectedQualityIndex = qualityOptions.length
    ? Math.max(0, Math.min(qualityOptions.length - 1, Math.floor(Number(selection) || 0)))
    : 0;
  const selectedQuality = qualityOptions[selectedQualityIndex] || null;
  const selectedQualityLabel = String(selectedAction?.label || selectedQuality?.label || parsed.qualityLabel || '最高可用画质').trim();
  const isDouyin = detectVideoProvider(parsed.sourceUrl)?.id === 'douyin';
  const pendingPreview = actionId !== 'backup' ? mediaPortalPreviewPromises.get(parsed) : null;
  if (pendingPreview) {
    runtimeLog(`parsed media waiting for active preview cache action=${actionId || 'video'}`);
    await pendingPreview.catch(() => null);
    if (mediaPortalParsedVideo !== parsed) return { ok: false, code: 'cancelled', reason: '解析结果已更新' };
  }
  runtimeLog(`parsed media selected action=${actionId || '-'} qualityIndex=${selectedQualityIndex} label=${selectedQualityLabel.slice(0, 120)}`);
  const reusableLocalPath = [parsed.capturedLocalPath, parsed.promotedLocalPath]
    .map((value) => String(value || ''))
    .find((value) => value && fs.existsSync(value));
  if ((!selectedAction || selectedKind === 'video') && actionId !== 'backup' && reusableLocalPath) {
    return promoteMediaPreviewToLibrary(parsed, downloadTarget, collection);
  }
  if (selectedAction?.id === 'backup' && !selectedAction.href && Number(selectedAction.candidateIndex ?? -1) < 0) {
    return { ok: false, code: 'manual-fallback', reason: '请在当前下载站手动选择备用下载', manual: true };
  }
  if (selectedKind === 'image') {
    return saveMediaPreviewCoverToLibrary(parsed, downloadTarget, collection);
  }
  if (selectedAction && selectedKind !== 'video' && !selectedAction.href && Number(selectedAction.candidateIndex ?? -1) < 0) {
    return { ok: false, code: 'media-action-unavailable', reason: `${selectedAction.label || '所选项目'}暂时不可下载` };
  }
  const view = ensureMediaPortalView();
  ...
  const waitForDownloadStart = (trigger, timeoutMs = 12000) => new Promise((resolve) => {
    ...
    const onDownload = (_event, item, sourceWebContents) => {
      ...
      const filenameKind = mediaKindForPath(item.getFilename());
      const urlKind = mediaKindForDownloadUrl(item.getURL());
      const mimeType = String(item.getMimeType() || '');
      const textualMime = /^(?:text\/|application\/(?:json|xml|xhtml\+xml))/i.test(mimeType);
      const kind = textualMime ? '' : (filenameKind || urlKind
  ...
  const actionDirectUrl = selectedAction?.href && isDirectMediaActionUrl(selectedAction.href) ? selectedAction.href : '';
  const selectedDirectUrl = selectedQuality?.href && isMediaUrl(selectedQuality.href) ? selectedQuality.href : '';
  const directUrl = actionDirectUrl || selectedDirectUrl || (selectedQualityIndex === 0 && parsed.qualityHref && isMediaUrl(parsed.qualityHref)
    ? parsed.qualityHref
    : '');
  ...
  try {
    ...
    const tryCandidates = async (candidateCount, firstPass = false, startIndex = selectedQualityIndex) => {
      ...
      for (let candidateIndex = firstCandidateIndex; candidateIndex < limit; candidateIndex += 1) {
        candidateResult = await waitForDownloadStart(async () => {
          ...
          if (href && isDirectMediaActionUrl(href)) {
            mediaPortalExpectedPopupDownloads.delete(view.webContents);
            startCapturedMediaPortalDownload(view.webContents, href);
          } else if (href) {
            runtimeLog(`ignored non-media download candidate href=${href.slice(0, 260)}`);
          }
          return { ok: true };
    ...
    const selectedCandidateIndex = Number.isInteger(selectedAction?.candidateIndex) && selectedAction.candidateIndex >= 0
      ? selectedAction.candidateIndex
      : selectedQualityIndex;
    if (!started.ok && parsed.downloadActionReady) {
      const candidateCount = Math.max(1, Math.min(8, Number(parsed.candidateCount || 3)));
      started = await tryCandidates(candidateCount, true, selectedCandidateIndex);
    }
    if (!started.ok && parsed.portalUrl && !isDouyin) {
      ...
      if (replay.ok) {
        const replayActions = Array.isArray(replay.mediaActions) && replay.mediaActions.length
          ? replay.mediaActions
          : mediaActions;
        const replayAction = replayActions.find((action) => action?.id === actionId) || null;
        ...
        mediaPortalParsedVideo = { ...parsed, ...replay, qualityOptions: replayQualityOptions, mediaActions: replayActions, requestId: parsed.requestId };
        const replayActionUrl = replayAction?.href && isDirectMediaActionUrl(replayAction.href) ? replayAction.href : '';
        const replayDirectUrl = replayActionUrl || (replaySelectedQuality?.href && isMediaUrl(replaySelectedQuality.href)
        ...
        const replayCandidateIndex = Number.isInteger(replayAction?.candidateIndex) && replayAction.candidateIndex >= 0
          ? replayAction.candidateIndex
          : replaySelectedIndex;
        if (!started.ok && replay.downloadActionReady) started = await tryCandidates(replay.candidateCount, false, replayCandidateIndex);
      } else {
        started = { ok: false, code: replay.reason, reason: replay.reason };
      }
    }
    if (!started.ok && selectedKind === 'video' && parsed.capturedLocalPath) {
      started = await promoteMediaPreviewToLibrary(parsed, downloadTarget, collection);
    }
    if (!started.ok && selectedKind === 'video' && /^(?:https?:|blob:)/i.test(previewFallbackUrl)) {
      started = await waitForDownloadStart(async () => {
...
function activateMediaPortalInContentRegion(view = mediaPortalView, durationMs = MEDIA_PORTAL_VIDEO_WAKE_VISIBLE_MS, label = 'portal') {
  if (!view || view.webContents.isDestroyed() || !mainWindow || mainWindow.isDestroyed()) return false;
  const bounds = mediaPortalLastVisibleBounds;
  if (!bounds || bounds.width < 120 || bounds.height < 80) {
    // Do not fall back to a full-window flash before the renderer has supplied the media frame.
    return activateMediaPortalInBackground(view, durationMs, label);
  }
  const restoreMainFocus = !!mainWindow.isFocused();
  ...
  view.webContents.setAudioMuted(true);
  try { view.webContents.focus(); } catch {}
  runtimeLog(`${label} embedded compositor wake`);
  setTimeout(() => {
    if (view !== mediaPortalView || view.webContents.isDestroyed() || mediaPortalRequestedVisible) return;
    keepMediaPortalWorkerVisible(view);
    if (restoreMainFocus && mainWindow && !mainWindow.isDestroyed()) {
...
function activateMediaPortalForTrustedSubmit(view = mediaPortalView, durationMs = MEDIA_PORTAL_VIDEO_SUBMIT_WAKE_MS) {
  return activateMediaPortalInContentRegion(view, Math.max(300, Number(durationMs || 0)), 'video parser trusted submit');
}

async function dispatchTrustedMediaPortalDownloadClick(view, fallbackPoint = {}) {
  if (!view || view.webContents.isDestroyed()) return false;
  activateMediaPortalInContentRegion(view, MEDIA_PORTAL_VIDEO_SUBMIT_WAKE_MS, 'video download trusted click');
  await new Promise((resolve) => setTimeout(resolve, 90));
  const refreshedPoint = await view.webContents.executeJavaScript(`(() => {
    const action = document.querySelector('[data-xuannian-download-action="true"]');
    if (!action) return null;
    action.scrollIntoView({ block: 'center', inline: 'center' });
    const rect = action.getBoundingClientRect();
    return { x: Math.round(rect.left + rect.width / 2), y: Math.round(rect.top + rect.height / 2) };
  })()`, true).catch(() => null);
  const point = refreshedPoint || fallbackPoint || {};
  const bounds = view.getBounds();
  const x = Math.max(1, Math.min(Math.max(1, Number(bounds?.width || MEDIA_PORTAL_WORKER_WIDTH) - 1), Math.round(Number(point.x || 0))));
  const y = Math.max(1, Math.min(Math.max(1, Number(bounds?.height || MEDIA_PORTAL_WORKER_HEIGHT) - 1), Math.round(Number(point.y || 0))));
  view.webContents.focus();
  ...
  view.webContents.sendInputEvent({ type: 'mouseDown', x, y, button: 'left', clickCount: 1 });
  view.webContents.sendInputEvent({ type: 'mouseUp', x, y, button: 'left', clickCount: 1 });
  runtimeLog(`media portal trusted download click point=${x},${y}`);
  return true;
}
...
function ensureMediaPortalView() {
  ...
  view.webContents.setWindowOpenHandler(({ url }) => {
    ...
    if (disposition === 'download') {
      consumeMediaPortalPopupDownload(view.webContents);
...
function setMediaPortalBounds(bounds = {}, visible = false, mode = 'browser') {
  ...
  const keepWorkerActive = !shouldShow && (!!mediaPortalInputState || !!mediaPortalPreviewCapture || activeMediaPortalDownloads > 0);
  if (shouldShow) {
    attachMediaPortalViewToMain(view);
    if (mediaPortalLastVisibleBounds) view.setBounds(mediaPortalLastVisibleBounds);
    view.setVisible(true);
  ...
  if (shouldShow) {
    cancelMediaPortalIdleDestroy();
    enforceMediaPortalCacheLimit(view.webContents);
  } else if (mediaPortalInputState || mediaPortalPreviewCapture || activeMediaPortalDownloads) {
    cancelMediaPortalIdleDestroy();
  } else {
    scheduleMediaPortalIdleDestroy();
  }
...
function openMediaPortal(url, downloadTarget = 'download', sourceText = '', autoSubmit = false, collection = '', qualityPreference = '', automationMode = '', originalSourceText = '') {
  const value = String(url || '').trim();
  ...
  const activeParsedVideo = normalizedAutomationMode === 'video-parse'
    && automationValue
    && mediaPortalParsedVideo?.sourceUrl === automationValue
    ? mediaPortalParsedVideo
    : null;
  const activePreviewCache = activeParsedVideo && (
    mediaPortalPreviewPromises.has(activeParsedVideo)
    || mediaPortalPreviewCapture?.sourceUrl === automationValue
  );
  if (activePreviewCache) {
    runtimeLog(`media portal ignored duplicate parse while preview cache is active request=${activeParsedVideo.requestId}`);
    updatePublishedMediaPortalVideo(activeParsedVideo);
    return { ok: true, requestId: activeParsedVideo.requestId, reusedActivePreview: true };
  }
  const view = ensureMediaPortalView();
  ...
  mediaPortalInputState = automationValue || normalizedAutomationMode === 'music-search' || normalizedAutomationMode === 'music-preview' || normalizedAutomationMode === 'music-download'
    ? {
      ...
      originalSourceText: normalizedAutomationMode === 'video-parse'
        ? String(originalSourceText || sourceText || '').trim()
        : '',
  ...
  if (cachedPreview && mediaPortalInputState && normalizedAutomationMode === 'video-parse') {
    ...
    const parsed = {
      ...
      mediaActions: Array.isArray(previousParsedVideo?.mediaActions) ? previousParsedVideo.mediaActions : [],
      reason: '',
      embeddedPreview: false,
      capturedDownloadUrl: '',
...
ipcMain.handle('media:openPortal', (_event, url, downloadTarget = 'download', sourceText = '', autoSubmit = false, collection = '', qualityPreference = '', automationMode = '', originalSourceText = '') => (
  openMediaPortal(url, downloadTarget, sourceText, autoSubmit, collection, qualityPreference, automationMode, originalSourceText)
));
...
ipcMain.handle('media:downloadParsedVideo', (_event, downloadTarget = 'download', collection = '', selection = 0) => {
  runtimeLog(`media:downloadParsedVideo invoked target=${downloadTarget === 'favorite' ? 'favorite' : 'download'} selection=${String(selection).slice(0, 32)}`);
  return downloadParsedMediaVideo(downloadTarget, collection, selection);
});
