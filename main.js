...
function notifyStickyPinState() {
  ...
  if (quickWindow && !quickWindow.isDestroyed() && !quickWindow.webContents.isDestroyed()) {
    quickWindow.webContents.send('native-sticky-pin-state', ids);
  }
}
...
async function pasteClipboardToActiveTarget() {
  ...
  await sleep(20);
  const targetFresh = lastPasteTargetHwnd && (!lastPasteTargetAt || Date.now() - lastPasteTargetAt < 15000);
  if (pendingCompositePaste?.text && pendingCompositePaste.files?.length) {
    ...
    if (process.platform === 'win32') {
      return activateWindowAndPaste(hwnd);
    }
  }
  if (process.platform === 'win32') {
    if (!lastPasteTargetHwnd || !targetFresh) return false;
    return activateWindowAndPaste(lastPasteTargetHwnd);
  }
...
`;
  ...
  child.stdout.on('data', (chunk) => {
    ...
    for (const line of lines) {
      ...
      if (action === 'QUICK') showQuickWindow();
...
function createQuickWindow() {
  ...
  quickWindow.on('blur', () => {
    setTimeout(() => {
      ...
      if (Date.now() - lastQuickToggleAt < 150) return;
      if (!quickWindow.isFocused()) quickWindow.hide();
    }, 80);
  });
...
function showQuickWindow() {
  const foregroundPromise = readForegroundWindowHandleAsync();
  ...
  quickWindow.setBounds({ x, y, width, height });
  quickWindow.setSize(width, height);
  if (!quickWindow.isVisible()) quickWindow.showInactive();
  quickWindow.webContents.send('quick:hotkey', hotkeyParts());
  if (quickWindow.webContents.isLoading()) {
    quickWindow.webContents.once('did-finish-load', () => {
      quickWindow.webContents.send('quick:hotkey', hotkeyParts());
      quickWindow.webContents.send('quick:refresh');
      quickWindowRevision = dataRevision;
    });
  ...
  foregroundPromise.then((foregroundHwnd) => {
    if (foregroundHwnd) {
      lastPasteTargetHwnd = foregroundHwnd;
      lastPasteTargetAt = Date.now();
    }
    if (quickWindow && !quickWindow.isDestroyed() && quickWindow.isVisible()) quickWindow.focus();
  });
}
...
function createStickyNoteWindow({ noteId = '', editNew = false, source = '' } = {}) {
  ...
  const initialBounds = clampBoundsToArea({
    width: size.width,
    height: size.height,
...
function broadcastDataRefresh(data = null, options = {}) {
  ...
  if (quickWindow && !quickWindow.isDestroyed() && !quickWindow.webContents.isDestroyed() && quickWindow.isVisible()) {
    quickWindow.webContents.send('native-clipboard-records', next.records);
    if (!options.recordsOnly) quickWindow.webContents.send('quick:refresh');
    quickWindowRevision = dataRevision;
  }
...
function copyFileToClipboard(filePaths, action = 'copy', text = '', options = {}) {
  return new Promise((resolve) => {
    ...
    const richImageComposite = !!clipboardText && files.every((filePath) => isImageFile(filePath));
    ...
    const fallbackPathText = clipboardText && files.length
      ? [clipboardText, ...files.filter((filePath) => !clipboardText.includes(filePath))].join('\n')
      : clipboardText;
    const encodedFiles = files.map((filePath) => Buffer.from(filePath, 'utf8').toString('base64'));
    const encodedText = fallbackPathText ? Buffer.from(fallbackPathText, 'utf8').toString('base64') : '';
    ...
    const script = [
      ...
      'if (-not $richImage) { $data.SetFileDropList($paths) }',
...
function normalizeClipboardRecord(record) {
  ...
  if (record.type === 'image') {
    ...
    if (!imageHash && (record.files || []).length === 1 && isImageFile(record.files[0])) {
      imageHash = imageHashFromFile(record.files[0]);
    }
...
async function captureClipboardToData(payload = null) {
  ...
  try {
    ...
    if (sequence && sequence !== claimedSequence) {
    ...
    if (sequence && (hasRecentClipboardSequence(sequence) || sequence === lastCapturedClipboardSequence || isSelfClipboardSequence(sequence))) {
      return false;
    }
...
function handleNativeClipboardSnapshot(line) {
  ...
  readClipboardPayload()
    .then((payload) => {
      let next = payload;
      if (nativeFilePayload && (!next?.files?.length)) next = nativeFilePayload;
      if (!next) return;
      const merged = {
        ...next,
        sequence: Number(next.sequence) || sequence,
        text: String(next.text || compositeText || '').trim(),
        action: next.action || nativeFilePayload?.action || 'copy',
      };
      captureClipboardToData(merged).catch(() => {});
    })
    .catch(() => {
      if (nativeFilePayload) captureClipboardToData(nativeFilePayload).catch(() => {});
    });
  return;
}
...
function startClipboardWatcher() {
  ...
  if (startNativeClipboardWatcher()) return;
  ...
  seedCurrentClipboardDigest()
    ...
    .finally(() => {
      ...
      fileClipboardTimer = setInterval(pollFileClipboard, 2500);
      imageClipboardTimer = setInterval(pollImageClipboard, 3500);
    });
}
